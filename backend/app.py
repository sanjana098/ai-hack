...
def generateResponse():
    ...
    res = openai.ChatCompletion.create(model="gpt-3.5-turbo",
                                                messages=[{"role": "user", "content": "Is there a medical condition in this sentence? Say only 1 if Yes or 0 if No - " +  prompt}])["choices"][0]["message"]["content"]
    ...
    server = mindsdb_sdk.connect('https://cloud.mindsdb.com', login=MINDSDB_EMAIL,password=MINDSDB_PASSWORD)
    ...
    medical_condition = openai.ChatCompletion.create(model="gpt-3.5-turbo",
                                                messages=[{"role": "user", "content": prefix_condition + prompt}])["choices"][0]["message"]["content"]

    print("Medical condition - ", medical_condition)
    input_embedding = openai.Embedding.create(input = [medical_condition.replace("\n", " ")], model="text-embedding-ada-002")['data'][0]['embedding']
    candidate_embeddings = openai.Embedding.create(input = s["Conditions"].tolist(), 
    ...
    cosine = np.sum(input_embedding*c_embeddings, axis=1)/(norm(c_embeddings)*norm(input_embedding))
    ind = np.argmax(cosine)
    record = s.loc[[ind]]

    new_prompt = "What are some adverse effects of this clinical trial"

    record_str = " " + str(record["Adverse Conditions"]) + " " + str(record["Outcome Measures"])
    res = openai.ChatCompletion.create(model="gpt-3.5-turbo",
                                                messages=[{"role": "user", "content": new_prompt + record_str}])["choices"][0]["message"]["content"]
    print(res)
